...
const Header = (props: { children: ReactNode; className?: string }) => {
  return (
    <header
      className={twMerge(
        'flex flex-col justify-center px-6 pt-6 pb-3 z-10 bg-transparent',
...
const Main = (props: { children: ReactNode; className?: string }) => {
  return (
    <main
      className={twMerge(
        clsx('grow overflow-y-auto p-6 pt-3 text-[color:var(--hm-ink)] bg-[color:var(--hm-bg)]', props.className),
      )}
    >
      {props.children}
    </main>
